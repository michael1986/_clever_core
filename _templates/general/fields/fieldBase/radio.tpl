<div class="form01FieldContainer">
    ...
    <div class="form01FieldField{?$error} form01FieldError{/?}">
        {*$options:$option}
        {?$_[option][queue]}<br>{/?}
        <input type="radio" id="{$option[id]}" name="{$name}" value="{$option[value]}"{?$option[selected]} checked="checked"{/?} class="{?$option[class]}{$option[class]}{!}form01FieldRadioInput{/?}"> <label for="{$option[id]}">{$option[label]}</label>
        {/*}
